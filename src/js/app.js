...
import stocksGrid from './stocks_grid.js'
...
$("#submit_form :submit").click(function (event) {
  ...
  // Let's call the webserver to ask for an authentication token in this demo we
  // call a login.js file that is an empty file and we'll fake the authentication 
  // on the client.
  $.ajax({
    url: "js/login.js",
    type: "POST",
    ...
    success: function () {
      // We expect the token to be sent by the WebServer, in this case the token
      // "generation" is simulated by the Authentication module.
      // Note that while the Authentication module will always return the same
      // token for a certain user, the WebServer would actually generate a
      // different token every time (or at least will refresh it from time to
      // time.
...
$("#logout").click(function () {
  ...
  // Clear the grids
  stocksGrid.clean();
  portfolioGrid.getDynaGrid().clean();
...
function startPortfolio(conn) {
  // This listener will be used to reroute updates from the JMS consumers to the UI
  var messageListener = {
    onMessage: function (message) {
